...
// ==================== 30 MISSÕES ====================
const MISSOES = [
    { id:0, titulo:"1. Contagem regressiva", historia:"Use um loop while para exibir os números de 5 até 1 (decrescente).", dica:"contador = 5; while contador > 0: print(contador); contador -= 1", codigoEsperado:["contador = 5","while contador > 0:","    print(contador)","    contador -= 1"] },
    { id:1, titulo:"2. Soma até 100", historia:"Some números de 1 até atingir ou ultrapassar 100. Exiba a soma final.", dica:"soma=0; num=1; while soma<100: soma+=num; num+=1; print(soma)", codigoEsperado:["soma = 0","numero = 1","while soma < 100:","    soma += numero","    numero += 1","print(soma)"] },
    ...
    { id:3, titulo:"4. Continue - Números ímpares", historia:"Exiba apenas números ímpares de 1 a 10. Use continue para pular pares.", dica:"i=1; while i<=10: if i%2==0: i+=1; continue; print(i); i+=1", codigoEsperado:["i = 1","while i <= 10:","    if i % 2 == 0:","        i += 1","        continue","    print(i)","    i += 1"] },
    { id:4, titulo:"5. AND - Múltiplo de 3 e 5", historia:"Percorra números de 1 a 20. Pare ao encontrar um número divisível por 3 e por 5.", dica:"i=1; while i<=20: if i%3==0 and i%5==0: break; print(i); i+=1", codigoEsperado:["i = 1","while i <= 20:","    if i % 3 == 0 and i % 5 == 0:","        break","    print(i)","    i += 1"] },
    { id:5, titulo:"6. OR - Parada com 0 ou negativo", historia:"Peça números até que o usuário digite 0 ou um número negativo. Use break.", dica:"while True: n=int(input()); if n==0 or n<0: break", codigoEsperado:["while True:","    num = int(input('Digite um número (0 ou negativo para parar): '))","    if num == 0 or num < 0:","        break","    print(f'Você digitou: {num}')"] },
    ...
    { id:8, titulo:"9. Break - Encontrando primo", historia:"Verifique se um número é primo. Se encontrar um divisor, use break.", dica:"num=int(input()); i=2; while i<num: if num%i==0: break; i+=1; else: print('primo')", codigoEsperado:["num = int(input('Digite um número: '))","i = 2","while i < num:","    if num % i == 0:","        print(f'{num} não é primo')","        break","    i += 1","else:","    print(f'{num} é primo')"] },
    ...
    { id:12, titulo:"13. Break e Continue combinados", historia:"Percorra 1 a 50. Pule pares (continue). Se primo maior que 30, pare (break).", dica:"Use while, if para par, continue, e break na condição especial.", codigoEsperado:["i = 1","while i <= 50:","    if i % 2 == 0:","        i += 1","        continue","    if i > 30 and i % 2 != 0 and i % 3 != 0 and i % 5 != 0:","        print(f'Primo encontrado: {i}')","        break","    print(i)","    i += 1"] },
...
io.on('connection', (socket) => {
    ...
    socket.on('roll-dice', ({ room, playerId }) => {
        ...
        // Envia o dado rolado
        io.to(room).emit('dice-rolled', { playerId, dice });
        // Avança o jogador de acordo com o dado (número de casas = missões a pular? Não, cada casa é uma missão)
        ...
        io.to(room).emit('move-result', { playerId, newPosition: newPos, positions: state.playersPositions, dice });
        // Verifica vitória
        ...
        // Envia a missão da casa onde caiu
    ...
    socket.on('submit-codigo', ({ room, playerId, posicao, blocosMontados }) => {
        ...
        let mensagem = acertou ? '✅ Código correto! Avançou uma casa.' : '❌ Código incorreto! Voltou uma casa.';
        // Atualiza posição
        if (acertou) {
            let newPos = state.playersPositions[idx] + 1;
            if (newPos > state.boardSize) newPos = state.boardSize;
            state.playersPositions[idx] = newPos;
            io.to(room).emit('move-result', { playerId, newPosition: newPos, positions: state.playersPositions, message: mensagem });
            if (newPos === state.boardSize) {
                state.gameActive = false;
                state.winner = playerId;
                io.to(room).emit('game-ended', { winner: playerId });
